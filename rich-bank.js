...
{
	if (typeof newAccountId !== "number" || !Number.isInteger(newAccountId) || newAccountId <= 0)
	{
		throw new Error("Invalid account ID. Please provide a positive value");
	}
	if  (typeof newAccountOwner !== "string" || newAccountOwner.trim() === "")
	{
		throw new Error("Invalid account owner. Please provide a non-empty string.");
	}

	const existingAccount = getAccountById(newAccountId);
	if (existingAccount)
	{
...
function depositMoney (accountId, amount)
{
	...
	{
		throw new Error("Invalid deposit amount. Please provide a positive finite number.");
	}
